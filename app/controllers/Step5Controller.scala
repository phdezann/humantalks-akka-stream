...
class Step5Controller(implicit system: ActorSystem) extends ControllerBase {
  ...
  override def index = Action {
    ...
    val pickEverySecond = Flow[JsValue].groupedWithin(1000, 1 second).map(_.last)
    ...
    val response = Source.fromGraph(GraphDSL.create() { implicit builder =>
      ...
      request ~> endpoint ~> toLines ~> toSensorValues ~> toJson ~> pickEverySecond ~> identity
